...
let Calculating(task: bool) =
    if task then
        ...
        let probNum1 = float num1 / float !k
        countProbability.[0] <- (num1, probNum1)
        let num2 = (Array.filter (fun elem -> elem  = 2) stream).Length
        let probNum2 = float num2 / float !k
        countProbability.[1] <- (num2, probNum2)
        let num3 = (Array.filter (fun elem -> elem  = 3) stream).Length
        let probNum3 = float num3 / float !k
        countProbability.[2] <- (num3, probNum3)
        let num4 = (Array.filter (fun elem -> elem  = 4) stream).Length
        let probNum4 = float num4 / float !k
        countProbability.[3] <- (num4, probNum4)
